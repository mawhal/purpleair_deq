...
# run the analysis for the PM2.5 data
# take the mean across all sensors and use this for comparison to DEQ
purp.mean <- purp %>% 
  group_by(time_stamp) %>% 
  summarize( pm25purp = mean(pm25purp))
...
# remove NA
dcomplete <- dcombine[ complete.cases(dcombine), ]
dcomplete$pm25deqlog <- log(dcomplete$pm25deq)
dcomplete$pm25purplog <- log(dcomplete$pm25purp)
# correlation test
cor.deq.purple <- cor.test( dcomplete$pm25deq, dcomplete$pm25purp )
cor.deq.purple$estimate
cor.deq.purple$conf.int
lm.deq.purple <- lm(pm25purp ~ pm25deq, data = dcomplete)
xylim <- range( c(dcomplete$pm25deq, dcomplete$pm25purp), na.rm = T )
svg("output/correlation_deq_purplemean.svg", width = 3, height = 3)
par(mar = c(5,4,2,2)+0.1, pty = "s", las = 1  )
...
####
# Log-log transform
lm.deq.purple.log <- lm(pm25purplog ~ pm25deqlog, data = dcomplete)
xylim <- range( c(dcomplete$pm25deqlog, dcomplete$pm25purplog), na.rm = T )
svg("output/correlation_deq_purplemean_log.svg", width = 3, height = 3)
par(mar = c(5,4,2,2)+0.1, pty = "s", las = 1  )
